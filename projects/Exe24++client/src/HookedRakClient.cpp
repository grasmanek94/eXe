...
Packet* HookedRakClientInterface::Receive( void )
{
	...
	Packet* p = g_RakClient->GetRakClientInterface()->Receive();
	BYTE packetId = GetPacketID(p);
	
	if (packetId == 77)
	{
		BitStream bsr(p->data, ((p->bitSize + 7) >> 3), false);
		...
		if (option < ((sizeof(Exe24Tool::CRemoteOptions)/sizeof(int))+1))
		{
			Exe24Tool::RemoteOptions->SetAt((size_t)option, (int)value);
		}
		else//bounce
		{
			...
			const unsigned long long PluginVersion = MakeVersion(2015, 1, 13, 11, 27);

			BitStream bs;
			bs.Write((unsigned char)77);
			bs.Write((unsigned char)66);
			bs.Write((bool)true);
			bs.Write((unsigned long long)PluginVersion);
#if defined PRIVATE
			bs.Write((bool)true);
#else
			bs.Write((bool)false);
#endif
			addMessageToChatWindow(D3DCOLOR_ARGB(0x77, 0, 255, 0), "Client: Za³adowano dodatek: eXe24+");
			g_RakClient->GetRakClientInterface()->Send(&bs, PacketPriority::HIGH_PRIORITY, PacketReliability::RELIABLE_ORDERED, 0);
		}
	}
	return p;
}
