	...
	if ( samp_dll != NULL )
	{
		...
		if ( g_dwSAMP_Addr != NULL )
		{
			...
			else
			{
				...
				MessageBox(NULL, "Ta wersja SA-MP lub GTA_SA.exe nie jest obs³ugiwana.\r\n\r\nProszê usun¹æ ten mod (plik d3d9.dll) z folderu GTA San Andreas.\r\n\r\nJedyna obs³ugiwana wersja SA-MP to Client 0.3z-R1\r\n\r\nSciagnij client z t¹d:\r\n\r\nhttp://files.sa-mp.com/sa-mp-0.3z-R1-install.exe\r\n\r\n\r\nPamiêtaj i¿ kontynuujesz grê na w³asne ryzyko", "B³¹d w ³adowaniu exe24 mod", MB_ICONEXCLAMATION);
				iIsSAMPSupported = 0;
				set.basic_mode = true;
...
void sampMainCheat ()
{
	...
	if (Exe24Tool::RemoteOptions->EnableRejoin)
	{
...
void addMessageToChatWindow ( int colorARGB, const char *text, ... )
{
	if ( g_SAMP != NULL )
	{
		va_list ap;
...
void addMessageToChatWindowSS ( const char *text, ... )
{
	if ( g_SAMP != NULL )
	{
		...
		addMessageToChatWindow(D3DCOLOR_ARGB(255, 0, 200, 200), tmp );
	}
	else
	{
		va_list ap;
