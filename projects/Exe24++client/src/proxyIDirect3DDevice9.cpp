...
void RenderMenu ( void )
{
	...
	strlcpy( title, "exe24 mod", sizeof(title) );
...
void renderSAMP(void)
{
	...
	if (!g_renderSAMP_initSAMPstructs)
	{
		...
		{
			ipcheck = true;
			std::string ip(g_SAMP->szIP);
			if ((ip.compare("91.121.149.219") != 0 && ip.compare("samp.exe24.info") != 0 && ip.compare("127.0.0.1") != 0) || (g_SAMP->ulPort != 7777 && g_SAMP->ulPort != 5482))
			{
				set.run_mode = RUNMODE_SINGLEPLAYER;
			...
			if ( /*cheat_panic() ||*/ cheat_state->state == CHEAT_STATE_NONE )
			{
				...
				/*if( !set.flickering_problem )
				{
					if ( iIsSAMPSupported )
					{
						...
						_snprintf_s( buf, sizeof(buf)-1, " ", "exe24 mod", g_szSAMPVer );//text herelol
					...
					else
					{
						...
						render->D3DBoxi( (int)x - 1,
										 (int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
										 (int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL );
						HUD_TEXT( x, D3DCOLOR_ARGB(127, 255, 255, 255), "exe24 mod" );
					}
				}*/
